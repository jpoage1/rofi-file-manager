...
class MenuManager:
    ...
    def filter_entries(entries, state, base_path=None, is_dir=False):
        filtered = []
        for e in entries:
            name = e.name if hasattr(e, "name") else e
            if not state.include_dotfiles and name.startswith('.'):
                continue
            if state.use_gitignore:
                # gitignore filtering logic here, or skip if not implemented
                pass
            if state.regex_mode:
                if not re.search(state.regex_pattern, name):
                    continue
            filtered.append(e)
        return filtered
    ...
    def dispatch_mode_action(self, resolved):
        if self.state.current_mode == "Edit":
            ...
            for path in resolved:
                p = Path(path)
                if p.is_dir():
                    self.state.root_dir = str(p.resolve())
                    self.state.mode = "NORMAL"
                    return  # Refresh menu with new directory contents
            # If no directory selected, optionally handle file selection here
